...
def get_active_or_max_academic_year(db: Session) -> int:
    ...
    Resolution order:
    1. Active RAW dataset period_end_year or academic_year with data.
    2. MAX(academic_year) from system.datasets WHERE is_analytics_enabled = true AND UPPER(COALESCE(workbook_type, 'RAW')) = 'RAW' AND COALESCE(row_count, 0) > 0.
    3. MAX(academic_year) from analytics.dashboard_agg.
    4. Any RAW dataset academic_year from system.datasets.
    5. Current calendar year as last-resort safety fallback.
    """
    try:
        # 1. Active RAW dataset with data
        row = db.execute(
            text("""
                ...
                ORDER BY is_period_active DESC, is_active DESC, academic_year DESC, created_at DESC
                LIMIT 1
            """)
        ).mappings().first()
        if row:
        ...
        # 2. Max academic_year from analytics-enabled RAW datasets
        row_max = db.execute(
            text("""
        ...
        # 3. Max academic_year from aggregate table
        row_agg = db.execute(
            text("""
                SELECT MAX(academic_year) as max_year
        ...
        # 4. Any RAW dataset academic_year
        row_any = db.execute(
            text("""
                SELECT MAX(academic_year) as max_year
                FROM system.datasets
        ...
        if row_any and row_any.get("max_year"):
            return int(row_any["max_year"])
