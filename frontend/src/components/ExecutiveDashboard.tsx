...
export const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({
  ...
  const [overview, setOverview] = useState<OverviewResponse | null>(() => dashboardCache.peekMemory<OverviewResponse>(initialOverviewKey));
  const [insights, setInsights] = useState<InsightItem[]>(() => dashboardCache.peekMemory<InsightItem[]>(initialInsightsKey) || []);
  const [monthlyTrend, setMonthlyTrend] = useState<MonthlyTrendItem[]>(() => dashboardCache.peekMemory<MonthlyTrendItem[]>(initialTrendKey) || []);
  const [rankings, setRankings] = useState<PerformanceRankingsResponse | null>(() => dashboardCache.peekMemory<PerformanceRankingsResponse>(initialRankingsKey));

  const [isLoading, setIsLoading] = useState<boolean>(() => !dashboardCache.peekMemory(initialOverviewKey));
  ...
  useEffect(() => {
    ...
    dashboardCache
      .fetchWithCache(ovKey, () => getDashboardOverview(currentFilters), { forceRefresh: force })
      .then((overviewData) => {
    ...
    dashboardCache
      .fetchWithCache(inKey, () => getDashboardInsights(currentFilters), { forceRefresh: force })
      .then((insightsData) => {
  ...
  useEffect(() => {
    ...
    dashboardCache
      .fetchWithCache(
        rankKey,
        () => getDashboardPerformanceRankings(rankingsDimension, currentFilters),
  ...
  useEffect(() => {
    ...
    dashboardCache
      .fetchWithCache(
        trendKey,
        () => getDashboardMonthlyTrend(mainMetric, currentFilters),
        { forceRefresh: loadVersion > 0 }
      )
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                {overview?.kpis?.leads?.py != null ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`font-semibold tabular-nums ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                      {(overview.kpis.leads.py ?? 0).toLocaleString()}
                    </span>
                  </div>
                ) : (
                ...
                <div className="font-extrabold text-indigo-500 dark:text-indigo-400 flex items-center gap-1">
                  ...
                  {overview.kpis.leads.sub_metric.py != null && (
                    <span className="text-[10px] font-normal text-slate-400 flex items-center gap-0.5">
                      ...
                      <span>{(overview.kpis.leads.sub_metric.py ?? 0).toLocaleString()}</span>
                    </span>
                  )}
                </div>
              </div>
                ...
                <div className="flex items-center justify-between text-xs pt-1">
                  {overview.kpis.cucet.py != null ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`font-semibold tabular-nums ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                        {(overview.kpis.cucet.py ?? 0).toLocaleString()}
                      </span>
                    </div>
                  ) : (
                    <span className={isDark ? "text-slate-400" : "text-slate-600 font-medium"}>Single Year Scope</span>
                  ...
                  <div className="font-extrabold text-violet-500 dark:text-violet-400 flex items-center gap-1">
                    ...
                    {overview.kpis.cucet.sub_metric.py != null && (
                      <span className={`text-[10px] font-normal ${isDark ? "text-slate-400" : "text-slate-600"} flex items-center gap-0.5`}>
                        ...
                        <span>{(overview.kpis.cucet.sub_metric.py ?? 0).toLocaleString()}</span>
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                {overview?.kpis?.admissions?.py != null ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`font-semibold tabular-nums ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                      {(overview.kpis.admissions.py ?? 0).toLocaleString()}
                    </span>
                  </div>
                ) : (
                  <span className={isDark ? "text-slate-400" : "text-slate-600 font-medium"}>Single Year Scope</span>
                ...
                <div className="font-extrabold text-blue-500 dark:text-blue-400 flex items-center gap-1">
                  ...
                  {overview.kpis.admissions.sub_metric.py != null && (
                    <span className={`text-[10px] font-normal ${isDark ? "text-slate-400" : "text-slate-600"} flex items-center gap-0.5`}>
                      ...
                      <span>{(overview.kpis.admissions.sub_metric.py ?? 0).toLocaleString()}</span>
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                {overview.kpis.conversion_rate.py != null ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`font-semibold tabular-nums ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                      {overview.kpis.conversion_rate.py}%
                    </span>
                  </div>
                ) : (
                  <span className={isDark ? "text-slate-400" : "text-slate-600 font-medium"}>Single Year Scope</span>
